...
/*
    ...
    Retrieving data, analyzing exposure, white balance, and updating metadata.
    This class is responsible to store its data in other classes, and mathematically calculate proper offsets to match exposure and temperature changes in a sequence of images (timelapse).
 */
public class proRAW {
    Util util;

    ArrayList<Image> images;
    ArrayList<Exposure_Change> exposureChanges;
    ArrayList<Temperature_Change> temperatureChanges;
    ...
    public proRAW() {
        ...
        exposureChanges = new ArrayList<Exposure_Change>();
        temperatureChanges = new ArrayList<Temperature_Change>();

        // Outputs welcome/introductory text
        welcome();
    ...
    public File getNextImage(String folder, Image currIMG) {
        ...
        // Increments large number
        // Uses it to get next file name
    ...
    public void getImageData(Image image) {
        ...
        // If it does not exist in the file... create white balance value
        else {
            int whiteBalance = getPrevImage(image).getWhiteBalance();
            image.setWhiteBalance(whiteBalance);

            String newData = util.createValue(xmp, "AutoWhiteVersion", "crs:Temperature=", Integer.toString(whiteBalance));
    ...
    // Analyzes exposure, searching for changes and creating "exposureChanges" lists, to store image data in change sequence
    // Then calculate the exposure offset required to match differences in images
    public void analyzeExposure() {
        ...
        for (int i = 0; i < images.size(); i++) {
            ...
            if (isExposureChanged(currImg)) {
                Exposure_Change newChange = new Exposure_Change(images, startExpoChange, i);
    ...
    // Analyzes white balance, searching for changes and creating "temperatureChanges" lists, to store change data in sequence
    // Then calculate the temperature offset required to match differences in images
    public void analyzeWB() {
        ...
        for (int i = 0; i < images.size() - 1; i++) {
            ...
            if (isWhiteBalanceChanged(currImg)) {
                ...
                Temperature_Change tempChange = new Temperature_Change(images, startWBChange, i);
    ...
    // Returns true if exposure setting on camera was changed between image and next image
    ...
    // Returns true if white balance was changed between image and next image
    ...
    public void printChanges() {
        for (int i = 0; i < exposureChanges.size(); i++) {
            ...
            System.out.print(NumToName(exposureChanges.get(i).getStartListNum()) + " - " + NumToName(exposureChanges.get(i).getLastListNum()));
            System.out.print(" (" + exposureChanges.get(i).getStartListNum() + " - " + exposureChanges.get(i).getLastListNum() + ")");
            System.out.println(" [" + exposureChanges.get(i).getTotalImages() + " images]");
            System.out.println("(" + exposureChanges.get(i).getExpoChange() + ")");
        ...
        for (int i = 0; i < temperatureChanges.size(); i++) {
            ...
            System.out.print(NumToName(temperatureChanges.get(i).getStartListNum()) + " - " + NumToName(temperatureChanges.get(i).getLastListNum()));
            System.out.print(" (" + temperatureChanges.get(i).getStartListNum() + " - " + temperatureChanges.get(i).getLastListNum() + ")");
            System.out.println(" [" + temperatureChanges.get(i).getTotalImages() + " images]");
            System.out.println("(" + temperatureChanges.get(i).getTemperatureChange() + ")");
            System.out.println(" [" + temperatureChanges.get(i).getIncrements() + "]");
        }
    }

    public void welcome() {
        System.out.println("========== Welcome to proRAW v1.0 ==========        by John Wonson");
        System.out.println();
        ...
        System.out.println();
        System.out.println("Please enter the file path to the first image");
    ...
    public void finish() {
        ...
        System.out.println("You may now open these files in after effects or media encoder to render the timelapse.");
        System.out.println("You can also open images in photoshop or lightroom to view changes.");
        System.out.println();
        System.out.println("- John Wonson");
    }
